...
import { OnboardingTourProvider } from '../tour/OnboardingTourProvider';
...
export default function AppShell() {
  ...
  const handleNavigate = (screen: Screen) => {
    ...
    recordEvent('navigation', screen, `Navigated to ${screen}`);
    storeBackendEvent.mutate({
      eventType: EventType.Navigation,
      elementId: screen,
      count: 1,
      payload: `Navigated to ${screen}`,
    });
  };
  ...
  return (
    <OnboardingTourProvider currentScreen={currentScreen}>
      <div className="flex min-h-screen flex-col bg-[#050505]">
        ...
        <main className="flex-1 overflow-x-hidden">
          ...
          {currentScreen === 'payment-failure' && (
            <PaymentFailureScreen onNavigate={handleNavigate} />
          )}
        </main>
