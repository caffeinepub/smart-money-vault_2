...
export default function SystemHeartbeatCard() {
  ...
  const { data: heartbeat, isLoading, isError, refetch } = useGetHeartbeatData(accountId);
  ...
  if (isError) {
    return (
      <div id="system-heartbeat-card" className="h-full rounded-xl border border-white/5 bg-[#121212] p-6">
        <StateMessage
          variant="error"
          title="Failed to Load"
          description="Unable to fetch system heartbeat data."
    ...
    <div id="system-heartbeat-card" className="h-full rounded-xl border border-white/5 bg-[#121212] p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-white/60">Bot Status</span>
          <div className="flex items-center space-x-2">
            ...
            <span className={cn('text-sm font-semibold', isOnline ? 'text-profit' : 'text-loss')}>
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-white/60">License</span>
          <div className="flex items-center space-x-2">
            {heartbeat?.verifiedLicense ? (
              <>
                <CheckCircle2 className="h-4 w-4 text-profit" />
                <span className="text-sm font-semibold text-profit">ACTIVE</span>
              </>
            ) : (
              <>
                <XCircle className="h-4 w-4 text-loss" />
                <span className="text-sm font-semibold text-loss">INACTIVE</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-white/60">ICP Cycles</span>
        ...
        <div className="flex items-center justify-between border-t border-white/5 pt-4">
          <span className="text-sm font-medium text-white/60">Last Heartbeat</span>
